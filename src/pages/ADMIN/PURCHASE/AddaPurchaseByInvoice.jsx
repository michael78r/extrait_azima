...
function PriceRow({ mypurchase, handleSacNumberChange, handleCtnNumberChange, handleBoxNumberChange, handleItemNumberChange, handleBottleNumberChange, handleDeleteItem, formatter, handleswitch, switchs }) {
    // const { mypurchase } = props;
    const [open, setOpen] = useState(false);

    // console.log("myp", mypurchase)
            ...
            <>
                <TableRow sx={{ '& > *': { borderBottom: 'unset' } }}>
                    ...
                    <TableCell align="right">
                        <Switch
                            checked={mypurchase.isActive}
                            onChange={handleswitch}
                            disabled={!mypurchase.isPromotion}
...
function AddaPurchaseByInvoice() {
    ...
    const [idInvoice, setIdInvoice] = useState(useParams().invoiceId);
    ...
    // 1. Chargement initial des promotions (une seule fois)
    useEffect(() => {
        axios.get(`/api/promotionsgoods`)
            .then((response) => {
                const prom = response.data;
                setPromotions(prom);
            })
            .catch(console.error);
    }, []); // ✅ Pas de dépendances
    
    // 2. Mise à jour des promotions quand purchaseValue OU promotions changent
    useEffect(() => {
        if (promotions.length > 0 && purchaseValue.length > 0) {
            const needsUpdate = purchaseValue.some(item => 
                item.isPromotion === undefined || 
                !promotions.some(p => 
            ...
            if (needsUpdate) {
                setPurchaseValue(prev => prev.map(item => ({
                    ...item,
                    isPromotion: promotions.some(p => 
                        p.brand.trim().toLowerCase() === item.brand?.trim().toLowerCase()
                    )
                })));
            }
        }
    }, [promotions, purchaseValue]); // ✅ Dépend des deux mais avec garde
    ...
    const handleSubmit = () => {
        ...
        setIsSaving(true);
        axios.post(`/api/addpurchase/${idInvoice}`, data)
                                ...
                                <TableBody>
                                    {purchaseValue.map((mypurchase) => (
                                        <PriceRow key={mypurchase.id}
                                            ...
                                            formatter={formatter}
                                            handleswitch={() => handleSwitch(mypurchase.id)}
                                        />
                                    ))}
                                </TableBody>
                            </Table>
...
export default AddaPurchaseByInvoice;